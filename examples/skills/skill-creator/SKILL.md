...
### Skill Location for CoDA Code

In CoDA Code, skills are stored in `~/.coda/<agent>/skills/` where `<agent>` is your agent configuration name (default is `agent`). For example, with the default configuration, skills live at:
...
For CoDA Code, use the agent's skills directory:

```bash
scripts/init_skill.py <skill-name> --path ~/.coda/agent/skills
```
