"""UI rendering and display utilities for CoDA Code."""

import json
from pathlib import Path
from typing import Any

from .config import COLORS, CODA_CODE_ASCII, MAX_ARG_LENGTH, console
...
def show_help() -> None:
    ...
    console.print("  coda [OPTIONS]                               Start interactive session")
    console.print("  coda list                                    List all available agents")
    console.print("  coda reset --agent <AGENT>                   Reset agent to default prompt")
    console.print("  coda reset --agent <AGENT> --target <SOURCE> Reset agent to copy of another agent")
    console.print("  coda help                                    Show this help message")
    console.print()
    ...
    console.print("  coda                                         Start with default agent", style=COLORS["dim"])
    console.print("  coda --agent mybot                           Start with agent named 'mybot'", style=COLORS["dim"],)
    console.print("  coda --model gpt-4o                          Use specific model (auto-detects provider)",
style=COLORS["dim"],)
    console.print("  coda -r                                      Resume most recent session", style=COLORS["dim"],)
    console.print("  coda -r abc123                               Resume specific thread", style=COLORS["dim"],)
    console.print("  coda --auto-approve                          Start with auto-approve enabled", style=COLORS["dim"],)
    console.print("  coda --sandbox runloop                       Execute code in Runloop sandbox", style=COLORS["dim"],)
    console.print()

    console.print("[bold]Thread Management:[/bold]", style=COLORS["primary"])
    console.print("  coda threads list                            List all sessions", style=COLORS["dim"])
    console.print("  coda threads delete <ID>                     Delete a session", style=COLORS["dim"])
    console.print()

    console.print("[bold]Skills Management:[/bold]", style=COLORS["primary"])
    console.print("  coda skills list [--project]                 List all or project skills", style=COLORS["dim"])
    console.print("  coda skills create <NAME> [--project]        Create a user or project skill", style=COLORS["dim"])
    console.print("  coda skills info <NAME>                      Show skill info", style=COLORS["dim"])
    console.print()
